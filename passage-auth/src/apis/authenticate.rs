...
mod tests {
    ...
    #[test]
    fn init_passage() {
        let config = Config::default().with_app_id("test".to_string());
        let passage = Passage::with_config(config);
    ...
    fn set_pub_jwk() {
        let passage = Passage::with_config(
            Config::default()
                .with_app_id("test".to_string())
                .with_api_key("fake_api_key".to_string()),
        );

        let passage = passage.set_pub_jwk(String::from(PUB_JWK));
    ...
    fn authenticate_good_token() {
        ...
        let config = Config::default()
            .with_app_id("fake".to_string())
            .with_api_key("fake_api_key".to_string())
            .with_pub_jwk(PUB_JWK.to_string());
        let passage = Passage::with_config(config);

        let res = passage.authenticate().authenticate_token(jwt_str);
